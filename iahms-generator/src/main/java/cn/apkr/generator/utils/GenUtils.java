...
public class GenUtils {
	...
	public static void initColumnField(GenTableColumn column, GenTable table) {
		...
		} else if (arraysContains(GenConstants.COLUMN_TYPE_NUM, dataType)) {
			column.setJavaType(GenConstants.COLUMN_TYPE_NUM_MAP.get(dataType));
...
//			}
		...
		// 编辑字段
		if (!arraysContains(GenConstants.COLUMN_NAME_NOT_EDIT, columnName) && !column.getIsPk()) {
			column.setIsEdit(GenConstants.REQUIRE);
		} else column.setIsEdit(GenConstants.NOT_REQUIRE);
		// 列表字段
		if (!arraysContains(GenConstants.COLUMN_NAME_NOT_LIST, columnName) && !column.getIsPk()) {
			column.setIsList(GenConstants.REQUIRE);
		} else column.setIsList(GenConstants.NOT_REQUIRE);
		// 查询字段
		if (!arraysContains(GenConstants.COLUMN_NAME_NOT_QUERY, columnName) && !column.getIsPk()) {
			column.setIsQuery(GenConstants.REQUIRE);
		} else column.setIsQuery(GenConstants.NOT_REQUIRE);
